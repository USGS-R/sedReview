...
count_sampleStatus <- function(x, bySite = TRUE) {
  ...
  if(bySite == TRUE){
  statusSum <- dplyr::summarise(dplyr::group_by(x,SITE_NO,STATION_NM,PARM_CD,PARM_NM,DQI_CD),
                                N = length(RESULT_VA)
  )
  statusSum <- reshape2::dcast(statusSum,SITE_NO+STATION_NM+PARM_CD+PARM_NM~DQI_CD,value.var = "N")
