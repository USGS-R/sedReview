# #'Merge Datasets. Direct copy from smwrBase to remove dependancy. Helper for get_UVflow.
# #'
...
merge_Nearest <- function(left, dates.left="DATES", all.left=FALSE,
                         suffix.left="left",
  ...
  ##    20180531 CPENN direct copy from smwrBase package to sedReview, commented out roxygen header
  ##
  ...
  if(all.left) {
    ...
    pck <- dplyr::recode(pck, 0, as.numeric(nrow(right)))
  }
  else {
