                     ...
                     resultAsText = FALSE)
{
  ...
  wideTable <- longTable[!(duplicated(longTable[c("RECORD_NO","PARM_CD")])),]
  wideTable <- reshape2::dcast(wideTable, RECORD_NO ~ PARM_CD,value.var = "Val_qual")
  ...
  names(wideTable) <- c("RECORD_NO", parmNames$PARM_NM[2:length(parmNames$PARM_NM)])
  
  #fill in record number meta data (station ID, name, date, time, etc)
  metaData <- unique(longTable[c("RECORD_NO","SITE_NO","STATION_NM","SAMPLE_START_DT","SAMPLE_END_DT","MEDIUM_CD",
  ...
  return(list(longTable=longTable,
              wideTable=wideTable))
