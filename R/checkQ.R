...
checkQ <- function(x, bySite = TRUE) {
  ...
  qRecords <- reshape2::dcast(qRecords,RECORD_NO~PARM_CD,value.var="RESULT_VA")
  
  sedRecords$hasQ <- ifelse(sedRecords$RECORD_NO %in% qRecords$RECORD_NO,TRUE,FALSE)
  
  Qsum <- dplyr::left_join(sedRecords,qRecords,by="RECORD_NO")
